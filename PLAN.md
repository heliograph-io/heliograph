...
| #174 | **Two of the three small fixes in the review roll-up** (#160) - the workflow is `Validate`, as in every skill repository, and it now calls the shared skill floor, whose ruff step found two unused imports in the Azure Function. `CODE_OF_CONDUCT.md` exists. `install.sh` no longer explains itself by a `${CLAUDE_SKILL_DIR}` SKILL.md stopped using. The site's `steps.md` said `lib/terraform.sh` applies and `lib/tfguard.sh` checks plans; neither is true, and a test now requires every helper row, there and in the skill's reference, to name functions its file defines. The MCP item is its own PR |
