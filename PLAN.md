...
| #163 | **The flagship `net-probe` example was refused, for the wrong reason** (#150) - `run.sh` registered the step as `net`, so the command every page leads with was refused on every stock station. `step_mode` dropped `run.sh`'s exit code, so "no such step" read as "declares no mode ()", and `status` and `watch` never printed the station's `reason:`. `net-probe` is now an alias, both stations refuse an unknown step as `unknown step`, and the CLI prints the reason. A test asks `run.sh` about every `heliograph send <step>` in the docs |
| #164 | **The skill's install command fetched a Linux amd64 binary on every machine and checked nothing** (#157) - on an Apple Silicon Mac or an arm64 Linux box it installed and chmodded cleanly, then failed with "exec format error", into a `/usr/local/bin` that often needs root. It now picks the asset from `uname`, compares it with the release's `SHA256SUMS` and installs into `~/.local/bin` only on a match. `tests/test-skill-install.sh` extracts the block from SKILL.md and runs it offline for all five platforms against fixtures, including a tampered file; a new `station.yml` job runs it live on Linux amd64 and arm64, macOS and Windows, and the installed binary has to run |
| #165 | **The skill promised the far side needs nothing beyond bash, git and coreutils** (#152) - choosing the relay, or starting a trusted set, puts `heliograph-seal` on a bash station, which refuses to start without it; and a busybox `sed` has only been a warning since the timestamp moved ahead of sed. SKILL.md now says what each choice costs the far side and points at `station/FAR-SIDE-BINARIES`. `test-doc-coherence.sh` reads that list and fails until SKILL.md names every entry, so the next binary cannot arrive without the skill saying so |
| #166 | **The skill never mentioned request mode, expiry or the trusted set** (#154) - the station refuses a request whose declared mode no longer matches the step and one past its 24 hours, and `doctor` points at `trust init`, so an agent could explain none of those to the person waiting. SKILL.md gains one short section, and the site's `send` section documents both flags. `test-doc-coherence.sh` builds the CLI from this tree and checks every flag SKILL.md passes to a verb against that verb's `--help`, and reads the 24 hours from `defaultExpiry` |
| #167 | **An agent shelled out to the CLI with the MCP tools loaded, and lost track of a request mid-`watch`** (#155) - SKILL.md named the MCP server only in its links, and showed a bare `watch`, which waits indefinitely while an agent's shell call does not. The coherence test kept `--timeout` off the page. SKILL.md now prefers the tools for what they cover and bounds every `watch`; the test requires `--timeout` on each and reads the tool names from `mcptools.go`, so a new tool fails it until the skill names it |
| #170 | **Ten of the skill's thirteen references had gone stale beside the site's tested copies** (#156) - none had changed since 12 Sep while the CLI gained trust, expiry and request mode, so an agent that opened one got the older answer: a busybox `sed` refused, the pre-station loop, the loop called an agent. `steps.md`, `secrets.md` and `method.md` (with `remote-repo.md` merged in) stay offline; the rest are links to `docs.heliograph.io/<page>.md`. The hosts page is now one copy, with the evidence table, and `test-hosts.sh` reads the site's. `test-doc-coherence.sh` refuses a fourth reference and checks every site link against `site/content/` |
| #169 | **`send` published to a branch no station reads** (#148) - a task branch inherits its parent's `station/status`, and a station still on the parent never reads it, so the request waited for nothing while `watch` and `doctor` read the parent's last run. `send` and `heliograph_send` now refuse when the status names another branch, `watch` warns and `doctor` fails. The refusal needed the station's half to be sound: a station publishes only when it runs something, so both twins now publish their own `starting` status when they start on a branch whose status names another, and only then. `station add` removes the status its new branch inherits. SKILL.md pushes the task branch before `plant` |
| #168 | **Four manifests sent readers to the retired host** (#151) - `plugin.json`, `server.json`, the npm `package.json` and the `.mcpb` manifest named `heliograph.dbhq.uk`, whose apex now 301s to the commercial site, and the coherence test passed only if SKILL.md named it too. All point at `docs.heliograph.io`, and a packaging test fails on the old host in any manifest, SKILL.md or shipped string. **Still open for a maintainer**: the MCP registry keeps the old `websiteUrl` until the next tagged release publishes `server.json`, and the old `io.github.dbhq-uk/heliograph` entry is still listed |
| #171 | **A second `send` silently replaced the first** (#159) - the station holds one request and the newest wins on every transport, while SKILL.md said a new send "queues". `send` and `heliograph_send` now name the unrun request they replace, from the same status read as the branch check, and SKILL.md and the CLI page say one slot, not a queue |
| #173 | **Every doc and template pointed at the station image from the deleted skill repository** (#158) - SKILL.md, three site pages, six Azure templates and the Kubernetes manifest defaulted to `ghcr.io/dbhq-uk/heliograph-toolkit:1.0.0-rc1`, and the Dockerfile said the station was built there. All now name `ghcr.io/heliograph-io/heliograph-toolkit:0.4.3`, which `publish-image.yml` built from the v0.4.3 tag, and a packaging test fails on the old name or a pinned tag with no release behind it. `publish-image.yml` now asks the question a stranger asks, an anonymous pull, and fails when the package is private. **Still open for a maintainer**: the package IS private (an anonymous token request answers 403), so those defaults do not pull until it is made public in the package's settings |
| #172 | **There was no way to cancel a step or stop the loop except by hand** (#153) - SKILL.md told an agent to edit `station/request`, commit and push, against its own rule, and meaningless on a share. `heliograph cancel [id]` and `heliograph stop`, with `heliograph_cancel` and `heliograph_stop`, set the field on the request already in the slot and keep its id: a new id would start a run once the cancel landed, and a blank step is the station's default step. `cancel` refuses where the station cannot read a request mid-run (bundle, object store, relay), and a test reads both stations' `live` capability to keep that list honest. End-to-end tests cancel a running step and stop the loop on git and a share. Found on the way: a bundle estate's carry instructions went to stdout, which is the MCP protocol stream, so they now go to stderr. The `.mcpb` manifest's tool list was a hand-typed seven in its test too, so it hid both tools while passing; the test now reads the names from `mcptools.go`. The Glama listing is behind by two more tools |
| #175 | **The MCP server could not say which tools only read, and an agent had no way to wait** (#160) - no tool declared `readOnlyHint` or `destructiveHint`, and the server spoke only `2024-11-05`, which has no annotations, so a client asked before every status read. Seven tools are now read-only and send, cancel and stop destructive; `initialize` answers the client's version among `2025-06-18`, `2025-03-26` and `2024-11-05`. `heliograph_wait(id, max_seconds)` is a bounded `watch` for an agent, with progress notifications when the call carries a token, and SKILL.md no longer says there is no watch tool. The Glama digest now covers annotations, and the listing is behind by one more tool |
